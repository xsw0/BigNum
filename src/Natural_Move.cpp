...
Natural Natural::operator>>(size_t offset) const {
  ...
  if (index_offset >= size || (index_offset == size - 1 && (data[size - 1] >> bit_offset) == 0)) {
    return {};
  }
...
Natural Natural::operator<<(size_t n) const {
  ...
  Natural natural;
  natural.reserve_data(size + index_offset + (bit_offset != 0));
  ...
  if (bit_offset == 0) {
    ...
    for (size_t i = 0; i != size; ++i) {
      natural[i + index_offset] = Binary::cat(prev, data[i], bits_width - bit_offset);
      prev = data[i];
    }
    natural[size + index_offset] = Binary::cat<uint_t>(prev, 0, bits_width - bit_offset);
    if (natural.size != 0 && natural[natural.size - 1] == 0) {
      --natural.size;
    }
    assert(natural.size == 0 || natural[natural.size - 1] != 0);
